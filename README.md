...
Relay is an AI-assisted classroom follow-up platform prototype. It turns class transcripts, pasted notes, roster details, and resource links into a teacher review workflow plus student-facing follow-up dashboards.
...
Relay runs as a desktop application. The hosted Vercel version also works as a mobile web app that can be added to a phone home screen.

```bash
./run.sh
```

The launcher installs or repairs missing dependencies with `npm ci` when `package-lock.json` is present, then opens Relay in a native Electron window.
...
- Teacher-assisted speaker matching for live capture: Relay creates unknown voice segments for review instead of using biometric voiceprints.
...
Relay should work without paid services. Transcript paste/upload, local accounts, review, publishing, student preview, analytics, and roster management are local-first.

External service support is intentionally narrow:

- Email: use a Gmail account you own, such as `relay.noreply@gmail.com`, with an app password. Relay cannot generate Gmail accounts or send from addresses you do not own.
...
- Canvas/LMS posting.
- External transcription-dependent online-call capture. Relay uses free browser capture/speech APIs where available and still treats platform transcript paste/upload as the reliable path.

## Hosted Backend And Freemium MVP

The local desktop app still works without paid services or cloud credentials. For a public browser version and multi-device access, Relay now includes a Supabase + Stripe backend scaffold:

- Supabase Auth for hosted teacher/student accounts.
- Supabase workspace state sync for browser/desktop continuity.
- Row Level Security SQL in `supabase/schema.sql` so each account can only read and write its own Relay state.
...
Relay includes a lightweight PWA layer for phones and tablets:
...
Relay creates Stripe Checkout sessions on the server. The React app does not need to embed a checkout form like Stripe's starter example; it calls `/api/billing/checkout`, receives a Checkout URL, and redirects the teacher there.

1. In Stripe, stay in **Test mode** while setting this up.
2. Go to **Product catalog** and create `Relay Pro`.
...
10. Set `RELAY_PUBLIC_URL` to your deployed app URL, such as `https://relay.vercel.app`.
...
4. Recreate or copy the `Relay Pro` product and `$9/month` recurring price in live mode.
...
Your Stripe publishable key starts with `pk_`. It is safe for browser code, but this current Checkout flow does not require it because the server creates the Checkout session. If Relay later adds Stripe Elements or client-side Stripe.js, add it as `VITE_STRIPE_PUBLISHABLE_KEY` in `.env.local` and Vercel.
...
Relay can package desktop installers with Electron Builder:
...
Relay is designed for teachers, tutors, instructors, club leaders, and students who need clean follow-up after a class or learning session.
...
5. Add a local backup/restore workflow for moving Relay data between devices without a hosted backend.
