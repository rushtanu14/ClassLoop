...
async function publishGeometrySample(page: Page) {
  ...
  await page.locator(".roster-name-field input").last().fill("Nina Park");
  await page.locator(".roster-email-field input").last().fill("nina@classloop.demo");
  await expect(page.getByLabel(/remove nina park/i)).toBeVisible();
...
test("teacher can resolve speakers, import, edit, preview, publish, and export", async ({ page }) => {
  await signIn(page, "teacher");

  await page.getByRole("button", { name: /new session/i }).first().click();
  await page.getByLabel(/session title/i).fill("Speaker Matching Check");
  await page.getByLabel(/paste transcript text/i).fill("[00:01] Maya iPad: I can explain the similar triangles proof.");
  await page.getByLabel(/bulk roster/i).fill("Maya Chen, maya@classloop.demo");
  await page.getByRole("button", { name: /generate draft/i }).click();
  await expect(page.getByText(/resolve transcript speakers/i)).toBeVisible({ timeout: 10_000 });
  await page.getByRole("button", { name: /link name/i }).click();
  await expect(page.getByText(/all transcript speakers match the roster/i)).toBeVisible();
  await page.getByRole("tab", { name: /follow-up/i }).click();
  await page.locator(".signal-row input[type='checkbox']").first().uncheck();
  await page.locator(".signal-row input[type='checkbox']").first().check();
...
test("privacy, sync billing, appearance, and tutorial controls are usable", async ({ page }) => {
  ...
  await expect(page.getByText(/sync & billing/i).first()).toBeVisible();
  ...
  await expect(page.getByText(/keep student data private/i)).toBeVisible();
  await page.getByLabel(/retention days/i).fill("180");
  await page.getByLabel(/require a consent notice/i).uncheck();
  await page.getByLabel(/allow teacher-controlled student data export/i).uncheck();
  await page.getByLabel(/no training on student data/i).check();
  await page.getByLabel(/what would make this better/i).fill("Add a clearer first-run checklist.");
  await page.getByRole("button", { name: /send feedback/i }).click();
  await expect(page.getByText(/feedback/i).last()).toBeVisible();
  ...
  await expect(page.getByText("pilot feedback", { exact: true })).toBeVisible();
  await expect(page.getByText(/privacy update/i).first()).toBeVisible();
});

test("student portal hides teacher tools and supports detail completion", async ({ page }) => {
  await signIn(page, "student");
  await expect(page.getByRole("button", { name: /analytics/i })).toHaveCount(0);
  await expect(page.getByRole("button", { name: /sync & billing/i })).toHaveCount(0);
  await expect(page.getByText(/follow-up dashboard/i)).toBeVisible();
  await page.getByRole("button", { name: /open detail/i }).click();
  await expect(page.getByText(/my next steps/i)).toBeVisible();
  await page.getByRole("button", { name: /complete check-in/i }).click();
  await page.getByRole("button", { name: /back to student dashboard/i }).click();
  await expect(page.getByText(/complete/i).first()).toBeVisible();

  await page.getByRole("button", { name: /how it works/i }).click();
  await expect(page.getByText(/learn how to use your student portal/i)).toBeVisible();
  await page.getByRole("button", { name: /^next/i }).click();
  await expect(page.getByRole("heading", { name: /read the latest recap/i })).toBeVisible();
});

test("core controls stay usable without horizontal overflow on a phone-sized viewport", async ({ page }) => {
